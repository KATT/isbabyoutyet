...
test("shows the hospital step as reached when at the hospital", async () => {
  await using _timers = useFakeTimersResource(new Date("2026-08-11T12:00:00.000Z"));
  ...
  expect(view.getByText(/6 hours ago/)).toBeTruthy();
  expect(view.getByText(/2 hours ago/)).toBeTruthy();
  expect(view.queryByText(/3 hours ago/)).toBeNull();
});
