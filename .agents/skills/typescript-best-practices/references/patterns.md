...
Code examples for each rule in `SKILL.md`. The underlying principles are language-agnostic; see the **type-system-discipline** and **boundary-discipline** principle skills. This repository's equivalents are the anti-slop oxlint plugin, `packages/runtime/src/guards.ts`, and `workspace/no-optional`.
...
Match the `readonly __brand: 'X'` shape; don't invent a new convention. Unique-symbol brands already used in query-prefetch handles are equivalent — keep that existing pattern.
...
Pick one discriminant name (`kind`, `type`, `tag`) and stick to it. This repo uses `type` for `BabyStatus` and `kind` for notification capability.
...
In this repo, keep `unknown` at the I/O boundary (`packages/runtime/src/guards.ts`). Ordinary functions take a named domain type — not `any` as a workaround for `anti-slop/no-unknown-parameters`.
...
This repo requires a `SAFETY:` comment on every non-`as const` assertion in production code.
...
Convex `v.*` validators and `packages/runtime/src/guards.ts` are the parse seams here.
...
Reach for `Pick`, `Omit`, `Parameters`, `ReturnType`, `Awaited`, `typeof` before writing a new interface. In this repo that usually means `Doc<"table">` and `FunctionReturnType<typeof api.module.fn>`.
...
Do not destructure the options object in the parameter list; access `opts.field`.
