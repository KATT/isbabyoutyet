...
export async function findActiveCoParent(
  ...
  const rows = await ctx.db
    .query("babyCoParents")
    .withIndex("by_babyId_userId", (q) => q.eq("babyId", babyId).eq("userId", userId))
    .collect();
  return rows.find(isActive) ?? null;
}

export async function canManageBaby(ctx: DbCtx, baby: Doc<"baby">, userId: string) {
  if (baby.userId === userId) return true;
  const coParent = await findActiveCoParent(ctx, baby._id, userId);
...
export async function requireBabyManager(ctx: DbCtx, babyId: Id<"baby">) {
  ...
  if (!isOwner) {
    const coParent = await findActiveCoParent(ctx, babyId, identity.subject);
    if (!coParent) {
