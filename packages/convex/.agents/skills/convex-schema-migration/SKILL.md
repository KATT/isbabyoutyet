...
Production example (#156): removing `learn_encouragements` from `onboardingStepIdValidator` while prod rows still contained that step ID blocked Vercel deploy until the validator was widened again.
...
For **adding** required fields, use the inverse: optional → backfill → required (see `migration-helper` and prior stacks like #125/#126, #135/#136).
...
- **PR 2/N:** Remove deprecated literals/fields from schema and TypeScript types after PR 1 has deployed and migrations finished.
...
For enum values in arrays, filter in `migrateOne` (see `sanitizeOnboardingSteps`).
...
Union literals and table fields both participate in deploy validation. When removing an onboarding step ID, restore the literal in `onboardingValidators.ts` with `@deprecated` until `sanitizeOnboardingSteps` has run:

```typescript
/** @deprecated Retired in #156; kept until sanitizeOnboardingSteps runs. */
...
- [ ] Phase 3 / PR 2/N: remove deprecated literals and schema fields
- [ ] pnpm checks
```
...
| --- | --- | --- |
| Remove `learn_encouragements` + `encouragementsDisabled` | Restore literal + `sanitizeOnboardingSteps` + `removeBabyEncouragementsDisabled` | Drop literal from `onboardingValidators.ts`, drop `encouragementsDisabled` from schema |
