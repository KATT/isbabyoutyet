...
import { Auth, DatabaseReader, DatabaseWriter, MutationCtx } from "./_generated/services";
...
const create = FunctionImpl.make(databaseSchema, encouragements, "create", (args) =>
  Effect.gen(function* () {
    ...
    const baby = yield* reader
      .table("baby")
      .get(args.babyId)
      .pipe(Effect.catchTag("GetByIdFailure", () => Effect.die(new Error("Baby not found"))));

    if (baby.encouragementsDisabled) {
      return yield* Effect.die(new Error("Encouragements are disabled for this baby"));
    }
    ...
    if (trimmedName.length > MAX_NAME_LENGTH) {
      return yield* Effect.die(new Error(`Name must be ${MAX_NAME_LENGTH} characters or less`));
    }
...
const update = FunctionImpl.make(databaseSchema, encouragements, "update", (args) =>
  Effect.gen(function* () {
    const reader = yield* DatabaseReader;
    const writer = yield* DatabaseWriter;
    ...
    const encouragement = yield* reader
      ...
      .pipe(
        Effect.catchTag("GetByIdFailure", () => Effect.die(new Error("Encouragement not found"))),
      );

    if (encouragement.visitorId !== args.visitorId) {
      return yield* Effect.die(new Error("Not authorized to edit this encouragement"));
    }

    if (!isWithinEditWindow(encouragement.createdAt)) {
      return yield* Effect.die(new Error("Edit window has expired (15 minutes)"));
    }
...
const listByBaby = FunctionImpl.make(databaseSchema, encouragements, "listByBaby", (args) =>
  Effect.gen(function* () {
    const reader = yield* DatabaseReader;
...
const remove = FunctionImpl.make(databaseSchema, encouragements, "remove", (args) =>
  Effect.gen(function* () {
    const reader = yield* DatabaseReader;
    const writer = yield* DatabaseWriter;
    ...
    if (!isOwner && !canVisitorDelete) {
      return yield* Effect.die(new Error("Not authorized to delete this encouragement"));
    }
